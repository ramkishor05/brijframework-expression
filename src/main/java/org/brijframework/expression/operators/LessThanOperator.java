...
public class LessThanOperator  extends Operators implements IOperator {
	...
	public Object   resultUnarySetup(Collection _array,OperatorMeta setup) {
		...
		for (int i = 0; i < aobj.length; i++) {
			if(OperatorUtil.lessThan(this.operationValue(aobj[i], setup.key),setup.param)){
				list.add(aobj[i]);
			}
		}
		return list;
	...
	public boolean state(Object ...objs) {
		return OperatorUtil.lessThan(objs[0],objs[1]);
	}
}
