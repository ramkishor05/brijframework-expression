...
public class ProductOperator extends Operators implements IOperator {
	...
	public Object   resultUnarySetup(Collection _array,OperatorMeta setup) {
		...
		if(aobj.length>0){
			objResult = OperatorUtil.bigDecimalForValue(this.operationValue(aobj[0], setup.key));
		}
		for (int i = 1; i < aobj.length; i++) {
			objResult=OperatorUtil.multiply(objResult, setup.param);
			objResult=aobj[i];
		}
		return objResult;
	}
